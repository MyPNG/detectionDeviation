...
1. Regulatory-First Evaluation: You will be provided with a `<target_policy_chunk>` and a `<gdpr_graph_context>` containing specific REG nodes and their 1-hop cross-references. You will also be provided with a list of `<main_entry_nodes>`.
2. The Task: You must evaluate EACH individual main REG node listed in `<main_entry_nodes>` against the ENTIRE policy chunk.
   - CRITICAL: For each main node, you MUST identify and include its 1-hop cross-references (provided in the graph relationships) in your analysis. Ask: "Does the policy satisfy the main REG node AND the additional constraints/details found in its referenced articles?"
...
4. Deviation Taxonomy: If a deviation is found, you MUST classify it into exactly one of the following categories. Read the examples carefully:
   - Completeness deviation: A major, mandatory topic, information, or constraint from the GDPR is entirely missing. (Example: Articles 13 & 14 require informing users about automated decision-making (ADM) logic. If the policy omits ADM entirely, it is a Completeness deviation.)
   - Data deviation: The specific scope, state, or category of data/processing is subtly altered or narrowed. (Example 1: GDPR grants access to data "being processed", but the policy limits it to data "stored" at rest. Example 2: GDPR grants the right to object specifically to "profiling," but the policy only mentions general "processing".)
   - Severity deviation: The policy is over-compliant (stricter about constraints than the GDPR requires). (Example: The GDPR requires informing a data subject within 72h, while the policy states it will inform them within 24h.)
...
5. Deliberation: You MUST think step-by-step inside <deliberation> tags BEFORE generating your final output.
6. Output Format: Output your final decision as a strict JSON array of objects inside a ```json block.
...
```text
<main_entry_nodes>
REG-001, REG-010, REG-013, REG-016, REG-017, REG-018, REG-021, REG-025, REG-026, REG-028, REG-029, REG-031, REG-032, REG-035, REG-040, REG-092
</main_entry_nodes>
