...
class ReaDeonticStagePipeline:
    """
    Run REA chunk requirements through stage 1-4 by reusing DeonticSlotExtractor:
    ...
    def __init__(
        ...
        self.extractor = DeonticSlotExtractor(
    ...
    def _fallback_stage3_from_stage2(self, stage2_output: list[dict[str, Any]]) -> list[dict[str, str]]:
        """
        Preserve REA units when the passive->active LLM call returns invalid JSON.
        Stage 4 expects Active_Recovered_Text, so we pass through the resolved text.
        """
        rows: list[dict[str, str]] = []
        for row in stage2_output:
            node_id = self._normalize_text(str(row.get("ID", "")))
            text = self._normalize_text(str(row.get("Text", "")))
            if not node_id or not text:
                continue
            rows.append({"ID": node_id, "Active_Recovered_Text": text})
        return rows
    ...
    def run_chunk_stage1_3(self, chunk_dir: Path, output_root: Path) -> dict[str, Any]:
        ...
        # If the LLM emits malformed JSON, keep the chunk alive by passing
        # stage2 text through unchanged. This avoids silently losing REA units.
        stage3_input = [{"ID": row["ID"], "Text": row["Text"]} for row in stage2_output if row.get("ID") and row.get("Text")]
        stage3_fallback_used = False
        stage3_fallback_error = ""
        try:
            stage3_output = self.extractor.make_passive_to_active(stage3_input)
        except Exception as exc:
            stage3_output = self._fallback_stage3_from_stage2(stage2_output)
            stage3_fallback_used = True
            stage3_fallback_error = str(exc)
        ...
        stage13_payload = {
            ...
            "stage3_fallback_used": stage3_fallback_used,
            "stage3_fallback_error": stage3_fallback_error,
        }
        ...
        if stage3_fallback_used:
            result["fallback_used"] = True
            result["fallback_reason"] = f"stage3 passive->active failed: {stage3_fallback_error}"
        return result
