...
class DeonticSlotExtractorLegacy:
    """
...
""".strip()
    ...
    def __init__(
        ...
        self.modal_rules_path = Path(__file__).resolve().parents[4] / "rules" / "modal_force_rules.txt"
        self.temporal_rules_path = Path(__file__).resolve().parents[4] / "rules" / "temporal_phrases_rules.txt"
    ...
    def _load_requirements(input_json: Path) -> list[dict[str, Any]]:
        ...
        def _load_modal_map_from_companion(report_path: Path) -> dict[str, str]:
            ...
            exact_extended = sorted(report_path.parent.glob("*_requirements_extended.json"))
            exact_main = sorted(report_path.parent.glob("*_requirements.json"))
            candidates.extend(exact_extended)
            candidates.extend(exact_main)
        ...
        if isinstance(payload, dict) and payload.get("test_type") == "passive_active_file_pipeline":
            ...
            for scenario in payload.get("results", []):
                ...
                for out_row in output_rows:
                    ...
                    text = DeonticSlotExtractorLegacy._normalize_text(
                ...
                if not output_rows:
                    ...
                    for node in input_nodes:
                        ...
                        text = DeonticSlotExtractorLegacy._normalize_text(str(node.get("Text", "")))
                        if not reg_id or not text:
                            continue
                        rows.append(
                            {
    ...
    def _extract_leading_condition(source_text: str) -> str:
        ...
        if not match:
            return ""
        return DeonticSlotExtractorLegacy._normalize_text(match.group(1))
    ...
    def _looks_positive_imperative(self, source_text: str) -> bool:
        ...
        - "Update and review ..."
        """
        text = DeonticSlotExtractorLegacy._normalize_text(source_text)
        if not text:
            return False
    ...
    def _is_trusted_actor_for_imperative(actor_hint: str) -> bool:
        ...
        """
        actor = DeonticSlotExtractorLegacy._normalize_text(actor_hint).lower()
        if not actor:
            return False
        trusted_markers = {
            "provider",
    ...
    def _is_already_active_modal_clause(source_text: str) -> bool:
        ...
        rest_after_first = DeonticSlotExtractorLegacy._normalize_text(modal_clause.group("rest") or "")
    ...
    def _has_explicit_subject_before_modal(text: str, modal_terms: list[str]) -> bool:
        clean = DeonticSlotExtractorLegacy._normalize_text(text)
        if not clean:
            return False
        ...
        if not match:
            return False
        subject_part = DeonticSlotExtractorLegacy._normalize_text(match.group(1))
    ...
    def _build_pipeline_nodes_from_group(group: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for row in group.get("nodes", []):
            ...
            node: dict[str, Any] = {
                ...
                "Text": DeonticSlotExtractorLegacy._normalize_text(str(row.get("Text", ""))),
            }
    ...
    def _clean_actor_candidate(actor: str) -> str:
        value = DeonticSlotExtractorLegacy._normalize_text(actor)
        if not value:
            return ""
        ...
        return DeonticSlotExtractorLegacy._normalize_text(value)
...
def main() -> None:
    ...
    extractor = DeonticSlotExtractorLegacy(
    ...
    #    .venv/bin/python .../DeonticSlotExtractorLegacy.py
    # 2) Passive->active single test run:
    #    .venv/bin/python .../DeonticSlotExtractorLegacy.py --test-passive-active
    # 3) Passive->active file pipeline run (stage1->stage2->post):
    #    .venv/bin/python .../DeonticSlotExtractorLegacy.py --run-passive-active-file
    # 4) Run all stages in one call (stage1->3 then stage4):
    #    .venv/bin/python .../DeonticSlotExtractorLegacy.py --run-all-stages
    if "--test-passive-active" in sys.argv:
        test_output = deontic_dir / f"{reg_input_name}_passive_to_active_test_result.json"
    ...
    if "--run-passive-active-file" in sys.argv:
        input_json = req_dir / f"{reg_input_name}_requirements_extended.json"
        output_json = deontic_dir / f"{reg_input_name}_requirements_extended_passive_active_report_llama3.json"
    ...
    if "--run-all-stages" in sys.argv:
        stage13_input = req_dir / f"{reg_input_name}_requirements_extended.json"
        stage13_output = deontic_dir / f"{reg_input_name}_requirements_extended_passive_active_report_llama3.json"
        ...
        stage4_output = deontic_dir / f"{reg_input_name}_requirements_extended_slots_llama3.json"
        saved_stage4 = extractor.run(input_json=stage13_output, output_json=stage4_output)
        print(f"Saved stage 4 slots: {saved_stage4}")
        return

    input_json = deontic_dir / f"{reg_input_name}_requirements_extended_passive_active_report_llama3.json"
    output_json = deontic_dir / f"{reg_input_name}_requirements_extended_slots_llama3.json"
