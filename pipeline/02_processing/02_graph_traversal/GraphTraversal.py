...
class GraphTraversal:
    ...
    def _load_graph(self) -> None:
        payload = load_json(self.graph_json_path)
    ...
    def _extract_main_constraints(self, reranked_payload: dict[str, Any], top_k: int) -> list[dict[str, Any]]:
        sorted_matches = get_sorted_top_matches_from_payload(reranked_payload)
    ...
    def build_graph_context_for_reranked(
        ...
        payload = load_json(reranked_path)
...
def main() -> None:
    ...
    def _resolve_graph_json() -> Path:
        ...
        fallback_candidates = [
            ...
            / "03_graphbuilder"
            / "requirementsgraphbuilder"
            / "reg_for_injectiontest"
            / "reg_for_injectiontest_requirements_graph.json",
            project_root
            / "intermediate_results"
            / "01_preprocessing"
            / "reg_prep"
            ...
            / "requirementsgraphbuilder"
            / "reg_eu_ai_act"
            / "reg_eu_ai_act_requirements_graph.json",
        ]
        for path in fallback_candidates:
            if path.exists():
                return path

        discovered = sorted((project_root / "intermediate_results" / "01_preprocessing" / "reg_prep").rglob("*requirements_graph.json"))
        if len(discovered) == 1:
    ...
    if input_path.is_dir():
        output_root = (
            ...
            else project_root / "intermediate_results" / "02_processing" / "02_graph_traversal" / "graphtraversal" / f"graph_context_{input_path.name}"
        )
