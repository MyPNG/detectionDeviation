...
from common.io_text import build_clause, load_json, normalize_spaces
...
Return JSON only using this exact schema:
{
  ...
  "overall_confidence": "low|medium|high"
    ...
    def __init__(
        ...
        """Initialize prompt builder with REG slot source and REA stage root."""
    ...
    def _normalize_spaces(value: Any) -> str:
        """Collapse whitespace for stable text comparison/rendering."""
        return normalize_spaces(value)

    @staticmethod
    def _build_clause(article: Any, paragraph: Any) -> str:
        """Build clause label in ArtX(Y) format when data is available."""
        return build_clause(article, paragraph)

    @classmethod
    def _clean_placeholder(cls, value: Any) -> str:
        """Remove missing-subject placeholder tokens from display text."""
    ...
    def _load_reg_rows(self) -> dict[str, dict[str, Any]]:
        """Load REG slot rows and index them by REG ID."""
    ...
    def _format_slot_block(row: dict[str, Any]) -> str:
        """Render one row of slot fields as multiline plain text."""
    ...
    def _sort_stage4_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Sort stage4 rows by REA id and split index for stable sibling selection."""
    ...
    def _resolve_stage_file(self, reranked_json_path: Path) -> Path:
        """Resolve matching REA stage file for a given reranked artifact."""
    ...
    def _load_focus_and_siblings(self, reranked_json_path: Path, sub_id: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        """Load focus REA unit and immediate siblings from stage4 output."""
    ...
    def _build_reg_context(self, top_matches: list[dict[str, Any]], top_k: int) -> tuple[list[str], list[str]]:
        """Build REG context blocks directly from reranked top matches."""
        blocks: list[str] = []
    ...
    def _format_graph_context_rows(self, rows: list[dict[str, Any]]) -> list[str]:
        """Render graph-neighbor rows into prompt-friendly bullet lines."""
        lines: list[str] = []
        for row in rows:
            if not isinstance(row, dict):
                continue
    ...
    def _build_reg_context_from_graph_context(self, graph_context: dict[str, Any]) -> tuple[list[str], list[str]]:
        """Build REG context blocks from precomputed graph traversal artifacts."""
        blocks: list[str] = []
        used_reg_ids: list[str] = []
    ...
    def build_prompt_payload(
        ...
        """Create one full prompt payload for a single REA sub-unit."""
    ...
    def _to_markdown(prompt_payload: dict[str, Any]) -> str:
        """Render prompt payload into a readable Markdown view."""
    ...
    def run(
        ...
        """Generate and save both JSON and Markdown prompt artifacts."""
...
def main() -> None:
    ...
    reranked_json = Path(reranked_json).expanduser().resolve()
    if not reranked_json.exists():
        raise FileNotFoundError(f"Set reranked_json to an existing file. Not found: {reranked_json}")
