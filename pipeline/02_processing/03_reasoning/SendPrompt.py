...
class SendPrompt:
    ...
    def __init__(
        ...
        """Initialize API client settings and prompt-cache behavior."""
    ...
    def _normalize_key_value(raw: Any) -> str:
        """Normalize env-var style key values, including quoted strings."""
    ...
    def _candidate_env_paths(self) -> list[Path]:
        """Build ordered candidate .env paths used for API key resolution."""
    ...
    def _load_api_key(self) -> str:
        """Load API key from .env files first, then process environment variables."""
    ...
    def _build_ssl_context(use_certifi: bool = False) -> ssl.SSLContext:
        """Create SSL context, optionally using certifi CA bundle."""
        if use_certifi:
            try:
                import certifi
    ...
    def _is_ssl_cert_error(exc: urllib.error.URLError) -> bool:
        """Detect certificate verification errors in URL layer failures."""
    ...
    def _extract_text_from_response(payload: dict[str, Any]) -> str:
        """Extract assistant text from OpenAI-compatible chat response payload."""
    ...
    def _post_json(self, body: dict[str, Any]) -> dict[str, Any]:
        """POST JSON to API with retry + SSL fallback behavior."""
    ...
    def send_prompt_json(
        ...
        """Send one prompt payload JSON and persist response JSON artifact."""
...
def main() -> None:
    ...
    prompt_cache_retention = "24h"  # "24h", "in_memory", or ""

    prompt_json = Path(prompt_json).expanduser().resolve()
    if not prompt_json.exists():
        raise FileNotFoundError(f"Set prompt_json to an existing file. Not found: {prompt_json}")
    ...
    sender = SendPrompt(
        ...
        enable_prompt_cache=bool(cache_system_prompt),
        prompt_cache_retention=str(prompt_cache_retention).strip(),
    )
    saved = sender.send_prompt_json(
        prompt_json_path=prompt_json,
        output_json_path=Path(output_json).expanduser().resolve() if output_json is not None else None,
